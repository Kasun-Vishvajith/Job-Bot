...
BUG FIXES applied:
  - Added `f_JT=I` filter param for "Internship" job type in intern searches.
  - Added `f_E=1,2` for entry-level & internship experience levels.
  ...
  - `f_TPR=r86400` (last 24h) changed to `f_TPR=r604800` (last 7 days) for more results.
    ...
    def scrape(self) -> list[dict]:
        ...
        for query_item in self.queries:
            ...
            for page in range(self.max_pages):
                start = page * 25
                # BUG FIX: Use f_TPR=r604800 (7 days) instead of r86400 (24h)
                # to get more results; 24h is too narrow and returns 0 most runs.
                # BUG FIX: Added f_E=1,2 (internship + entry-level experience filter).
                url = (
                    f"{LINKEDIN_BASE}?keywords={requests.utils.quote(query)}"
                    f"&start={start}&f_TPR=r604800&f_E=1%2C2"
                )
                if location:
                ...
                # Tag remote work_type when location param is "Remote"
                if location:
                ...
                # BUG FIX: Sleep was after break check so it ran even on last page.
                # Now it only sleeps when there are more pages to fetch.
