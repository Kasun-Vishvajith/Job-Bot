...
BUG FIXES applied:
  ...
  - `fromage=7` changed to `fromage=14` for more results (7 days is often too narrow).
  - Indeed card selector `[data-jk]` often captures parent AND child; added dedup guard.
  - `_make_id` was based on title+company+location, causing duplicates across pages
    since location might differ slightly. Now uses job_id (data-jk) when available.
  - Added country-specific URL support (in.indeed.com for India, lk.indeed.com etc.)
    ...
    def scrape(self) -> list[dict]:
        ...
        for query_item in self.queries:
            ...
            for page in range(self.max_pages):
                start = page * 10
                # BUG FIX: fromage=14 (14 days) for more results; 7 often returns 0.
                url = (
