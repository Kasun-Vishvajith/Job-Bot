"""
Glassdoor Job Scraper — REMOTE-ONLY MODE
...
REMOTE-ONLY:
  - Uses remoteWorkType=1 in search params to filter for remote jobs.
  - Post-parse enforcement: rejects any job not detected as Remote.
"""
    ...
    def scrape(self) -> list[dict]:
        ...
        for query_item in self.queries:
            ...
            for page in range(self.max_pages):
                ...
                # Only apply remote filter for remote-specific queries
                if is_remote_query:
                ...
                if is_remote_query:
                    # Post-parse enforcement — reject non-remote only for remote queries
                    remote_jobs = [j for j in page_jobs if j.get("work_type") == "Remote"]
                    rejected = len(page_jobs) - len(remote_jobs)
                    if rejected > 0:
                        log.debug("  [Glassdoor] Rejected %d non-remote jobs", rejected)
                    jobs.extend(remote_jobs)
                    log.debug("  [Glassdoor] Page %d: %d remote listings", page + 1, len(remote_jobs))
                else:
                    # Local query (e.g. Sri Lanka) — keep all work types
